...
possible_maize_areas <- function(iso3='TZA') {
    ...
    acy_exclude <- (acy_fut - acy_cur) / acy_cur >= -.05

    acy_exclude_poly <- rasterToPolygons(acy_exclude)
    ...
    ygap_ratio <- raster(file.path(data_base, 'HarvestChoice', 'gap2000_r_mze_2000_yga_cl.tif'))
    ygap_ratio <- setup_raster_layer(ygap_ratio)

    # Difference of potential and actual production for rain-fed maize
    ygap_diff <- raster(file.path(data_base, 'HarvestChoice', 'gap2000_r_mze_2000_qga.tif'))
    ygap_diff <- setup_raster_layer(ygap_diff)

    ygap_diff_poly <- rasterToPolygons(ygap_diff)
